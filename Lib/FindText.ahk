...
ShowScreenShot(x1:=0, y1:=0, x2:=0, y2:=0, ScreenShot:=1)
{
  ...
  IfWinNotExist
  {
    ...
    Gui, +LastFound +AlwaysOnTop -Caption +ToolWindow -DPIScale +E0x08000000 +OwnDialogs
    Gui, Margin, 0, 0
...
Gui(cmd, arg1:="", args*)
{
  ...
  Case "Test","TestClip":
    ...
    else
    {
      ...
      , RegExMatch(s, "O)<[^>\n]*>[^$\n]+\$[^""\r\n]+", r)
      , v:=this.FindText(X, Y, -n, -n, n, n, 0, 0, (r?r[0]:""))
      , r:=StrSplit(Lang["s8"] "||||", "|")
  ...
  Case "GetOffset", "GetClipOffset":
    ...
    if (cmd="GetOffset")
      ...
      GuiControlGet, s,, ClipText
    RegExMatch(s, "O)<[^>\n]*>[^$\n]+\$[^""\r\n]+", r), n:=150000
    if this.FindText(X, Y, -n, -n, n, n, 0, 0, (r?r[0]:""))
    {
      ...
      Loop % w:=nW-CutLeft-CutRight
      {
        ...
        While InStr(v,bg)
        {
          if (v~="^" bg "+\n")
            ...
            v:=RegExReplace(v,"\n\K" bg "+\n$")
          ; 该文件存在于由风屿 FYWinds i@windis.cn 编写的自动钓鱼脚本中
          else if !(v~="m`n)^[^\n" bg "]")
            v:=RegExReplace(v,"m`n)^" bg)
          else Break
        }
